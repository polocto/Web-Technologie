...
module.exports = {
  ...
  messages:{
    create: async(message) => {
      ...
      await db.put(`channel:${channel.id}:message:${creation}`)
    ...
    list: async() => {
      ...
      return Promise ((resolve, reject)=>{
        const messages= []
        db.createReadStream({
          gt: `channel` + String.fromCharCode(":".charCodeAt(0) + 1)+`:message:`,
          lte: `channel` + String.fromCharCode(":".charCodeAt(0) + 1)+`:message` + String.fromCharCode(":".charCodeAt(0) + 1),
        }).on( 'data', ({key, value})=>{
          console.log("Victoire !!")
          message = JSON.parse(value)
          console.log(key.split(':')[0]+key.split(':')[1]+key.split(':')[2])
          message.id = key.split(':')[3]
